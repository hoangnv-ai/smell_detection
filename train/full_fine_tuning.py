...
log_file_path = os.path.join(args.output_dir, model_name, f"full_fine_tuning_seed_{args.seed}", f"train_max_samples_{args.max_train_samples}_log.txt")
...
with open(f"{args.output_dir}/{model_name}/peak_memory.txt", "a") as f:
    f.write(f"full fine-tuning: {str(peak_memory)}")
...
with open(f"{args.output_dir}/{model_name}/training_time.txt", "a") as f:
    f.write(f"epoch: {args.num_epochs} full fine-tuning: {str(training_time)}")