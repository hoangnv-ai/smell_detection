...
parser.add_argument("--train_file", type=str, default="dataset/trainset_CC.csv")
parser.add_argument("--valid_file", type=str, default="dataset/validset_CC.csv")
...
log_file_path = os.path.join(args.output_dir, model_name, f"ia3_seed_{args.seed}", f"train_max_samples_{args.max_train_samples}_log.txt")
...
def tokenize_function(examples):
    outputs = tokenizer(examples["text"], truncation=True, padding="max_length", max_length=max_seq_length)
    return outputs
...
for epoch in range(args.num_epochs):
    ...
    if avg_validation_loss < best_validation_loss:
        ...
        best_model_path = os.path.join(args.output_dir, model_name, f"ia3_seed_{args.seed}", "best_model")
    ...
    save_path = os.path.join(args.output_dir, model_name, f"ia3_seed_{args.seed}", f"epoch_{epoch + 1}")
    os.makedirs(save_path, exist_ok=True)
    model.save_pretrained(save_path)

with open(f"{args.output_dir}/{model_name}/peak_memory.txt", "a") as f:
    f.write(f"ia3: {str(peak_memory)}")
...
with open(f"{args.output_dir}/{model_name}/training_time.txt", "a") as f:
    f.write(f"epoch: {args.num_epochs} ia3: {str(training_time)}")